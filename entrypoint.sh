...
: ${DAEMON_NAME:="terrad"}
: ${DAEMON_VERSIONS:="2.0.0-rc.1"}
: ${DAEMON_HOME:="${HOME}/.${DAEMON_NAME}"}
: ${DAEMON_PREFIX:="$(echo ${DAEMON_NAME} | sed -e 's/.$//')"}
: ${GITHUB_REPO:="terra-money/core"}
...
one_offs(){
    if [ "${DAEMON_NAME}" = "terrad" ]; then
        terrad_one_off
    fi
}

# ONE OFF FIX FOR TERRA AND RLS 2.2.0 BUILT DYNAMICALLY
terrad_one_off(){
    if [ -f "${UPGRADE_JSON}" ]; then
        sed -e "s/2.2.0/2.2.1/" -i "${UPGRADE_JSON}"
    fi
}

# Download the binaries for the given upgrades
download_binaries(){
    local upgrade
    for upgrade in ${DAEMON_VERSIONS}; do
        download_binary "${upgrade}"
        link_cv_genesis "${upgrade}"
    done
    link_cv_current "${upgrade}"
}
...
download_binary(){
    ...
    if [ ! -f "${binary}" ]; then
        mkdir -p "${bin_path}"
        local download_url="${DOWNLOAD_URL}/v${upgrade}/${DAEMON_PREFIX}_${upgrade}_${OS}_${ARCH}.tar.gz"
        wget "${download_url}" -O- | tar xz -C "${bin_path}"
    fi
}
...
download_genesis(){
    ...
    if [ -n "${GENESIS_URL}" ] && [ ! -f "${GENESIS_FILE}" ]; then
